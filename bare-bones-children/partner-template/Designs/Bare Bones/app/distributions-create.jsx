...
// Funding stamps the record date and locks the targeted classes (handled by the
// parent on submit). Mirrors create(slug, shareToken, classIds[], ratesPerShare[], amount).

function CreateDistributionModal({ activeDao, chain, onClose, onCreate }) {
  const [label, setLabel] = React.useState('');
  const [mode, setMode] = React.useState('prorata');
  const [classIds, setClassIds] = React.useState(['common']);
  const [pool, setPool] = React.useState('60000');
  const [rate, setRate] = React.useState('0.50');
  ...
  const eligible = CAP_HOLDERS
    .filter(h => classIds.includes(h.classId) && h.grantStatus === 'Active' && holderBasisShares(h) > 0)
    .sort((a, b) => holderBasisShares(b) - holderBasisShares(a));
  const previewDist = { ...draft, pool: poolN, ratePerShare: rateN, token };
  const topHolders = eligible.slice(0, 3);

  // Name is optional — fall back to a sensible default so the action never
  // silently no-ops. Only a target class + a positive amount are truly required.
  const amountOk = mode === 'prorata' ? poolN > 0 : rateN > 0;
  const valid = classIds.length > 0 && selectedBasis > 0 && amountOk;
  const blockReason =
    classIds.length === 0 ? 'Pick at least one target class'
    : selectedBasis === 0 ? 'Selected classes have no payable holders'
    : !amountOk ? (mode === 'prorata' ? 'Enter a pool amount' : 'Enter a per-share rate')
    : null;
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      onCreate({
        label: label.trim() || fallback, mode, classIds, token,
    ...
    <div className="modal-scrim" onClick={onClose}>
      <div className="modal dist-create-modal" onClick={e => e.stopPropagation()}>
        <div className="modal-head">
          <div>
            <div className="modal-kicker">Distributions · {activeDao?.name}</div>
        ...
        <div className="dist-create-body">
          <div className="field full">
            <label>Name <span className="muted" style={{ textTransform: 'none', letterSpacing: 0 }}>· optional</span></label>
          ...
          {/* target classes */}
          <div className="dist-field-block">
            <div className="dist-field-label">Target classes <span className="muted">· who gets paid</span></div>
                ...
                return (
                  <button key={c.id} className={`dist-class-opt${on ? ' on' : ''}`} onClick={() => toggleClass(c.id)}>
                    ...
                    <span className="dist-class-opt-k">
                      <span className="dist-class-opt-name"><span className="dist-chip-dot" style={{ background: c.color }} />{c.name}</span>
                      <span className="dist-class-opt-sub mono">{classBasisLabel(c)} · {fmtShares(distClassBasis(c.id))} sh basis</span>
                    </span>
                  </button>
                );
              })}
          ...
          <div className="dist-summary">
            ...
            {topHolders.length > 0 && (
              <div className="dist-summary-top">
                <div className="kicker" style={{ marginBottom: 6 }}>Top holders get</div>
                {topHolders.map(h => (
                  <div key={h.id} className="dist-summary-holder">
                    <span className="dh-avatar sm" style={{ background: `oklch(0.7 0.12 ${h.avatarHue})` }}>{h.initials}</span>
                ...
                {eligible.length > topHolders.length && (
                  <div className="muted small" style={{ marginTop: 4 }}>+ {eligible.length - topHolders.length} more holder{eligible.length - topHolders.length === 1 ? '' : 's'}</div>
                )}
              </div>
            )}
          </div>
          ...
          <div className="cd-note accent">
            <I.Info size={14} />
            <div>Confirming funds the pool ({token} approve + <span className="mono">create</span>), stamps the <b>record date</b>, and <b>locks</b> the targeted classes until every holder is paid.</div>
          </div>
        </div>

        <div className="modal-foot">
          {blockReason
            ? <span className="muted small" style={{ marginRight: 'auto', display: 'inline-flex', alignItems: 'center', gap: 6 }}><I.Info size={13} /> {blockReason}</span>
            : <span className="muted small" style={{ marginRight: 'auto' }} />}
          <button className="btn-ghost btn-sm" onClick={onClose} disabled={funding}>Cancel</button>
          <button className="btn-primary btn-sm" disabled={!valid || funding} onClick={submit}>
            {funding ? <><span className="spinner sm" /> Funding…</> : <>Fund {fmtMoney(totalOut, token)}</>}
          </button>
        </div>
      </div>
    </div>
...
Object.assign(window, { CreateDistributionModal });
