...
function QuoteRow({ q, listing, pov, phase, actions, requireWallet }) {
    ...
    <div className={`lm-quote${q.mine ? ' mine' : ''}`}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          ...
          {q.status !== 'pending' && <span className={`pay-status pay-status-${s.tone}`} style={{ padding: '2px 8px', fontSize: 10.5 }}><span className="pay-status-dot" />{s.label}</span>}
        </div>
...
function ListingDetail({ listing: l, pov, wallet, activeDao, onBack, actions, requireWallet, openModal }) {
  ...
  const docsUnlocked = (pov === 'borrower' && isOwner) ||
  ...
  const sortedQuotes = [...l.quotes].sort((a, b) => {
    const order = { pending: 0, accepted: -1, funded: -1 };
      ...
      <div className="lm-detail-grid">
            ...
            <div className="panel-body">
              <div className={`lm-doc${docsUnlocked ? '' : ' locked'}`}>
                ...
                <div className="lm-doc-k">
                  ...
                  {docsUnlocked ? (
                    <a className="cfg-link" href="#" onClick={(e) => { e.preventDefault(); window.toast.info('Opening secure link', { description: l.docLink, duration: 3000 }); }} style={{ marginTop: 2 }}>
        ...
        <div className="lm-rail">
          <ActionRail
            ...
            matchedQuote={matchedQuote} myPending={myPending} loanMath={m}
...
// ===================================================================
function ActionRail({ listing: l, phase, pov, isOwner, matchedQuote, myPending, loanMath: m, actions, requireWallet, openModal, wallet }) {
  // ---- LENDER ----
  if (pov === 'lender') {
    if (phase === 'open') {
      return (
    ...
    if (['funded', 'grace', 'defaulted'].includes(phase) && l.loan && l.loan.mine) {
  ...
  if (phase === 'open') {
    return (
      <div className="lm-action accent">
        ...
        <div className="lm-action-sub">{l.quotes.filter(q => q.status === 'pending').length} lender{l.quotes.filter(q => q.status === 'pending').length === 1 ? '' : 's'} competing. Accept one in the quote book to lock terms and release documents — no funds move yet.</div>
  ...
  if (phase === 'matched') {
    return (
      <div className="lm-action accent">
        ...
        <div className="lm-action-sub">{matchedQuote?.lender.name} was accepted at <b>{fmtUsd(matchedQuote?.amount)}</b> @ {bpsPct(matchedQuote?.rateBps)}. They're reviewing documents before placing principal in escrow.</div>
  ...
  if (['funded', 'grace', 'defaulted'].includes(phase)) {
    return (
      <div className={`lm-action accent`}>
...
Object.assign(window, { ListingDetail, ActionRail, QuoteRow, ClosedRail });
