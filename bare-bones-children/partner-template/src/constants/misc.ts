...
// Single source of truth for which chains the build exposes. Every chain is
// registered unconditionally in the maps below; this list controls visibility
...
export const MOCK_GOVERNANCE_TOKEN_BY_CHAIN: Partial<Record<number, string>> = {
  [POLYGON_CHAIN_ID]: "0xe4368424E6728F8D53Ed524eE540FA8f0595dF43",
  [LOCAL_CHAIN_ID]: import.meta.env.VITE_LOCAL_MOCK_GOVERNANCE_TOKEN_ADDRESS,
  [STAGING_CHAIN_ID]: import.meta.env.VITE_STAGING_MOCK_GOVERNANCE_TOKEN_ADDRESS,
};
...
export const LOCAL_SECURE_VALUE_RESERVE_GRAPH_URL =
...
// Staging chain has its own deployment with deterministic-but-different
// addresses from any mainnet. We mirror the LOCAL_* env-var pattern with
// VITE_STAGING_* keys so the frontend can be pointed at staging via env
// without hardcoding addresses that drift on every fresh deploy. Populate
// the values in `.env.staging.local` (gitignored, generated by the
// staging-deploy flow from `.env.deploy.anvil.generated`).
const STAGING_BARE_BONES_CONFIG_ENV_KEYS: Record<Exclude<keyof BareBonesConfiguration, "svrFactoryAddress">, string> = {
  diamondFactoryAddress: "VITE_STAGING_DIAMOND_FACTORY_ADDRESS",
  diamondFactoryInitHash: "VITE_STAGING_DIAMOND_FACTORY_INIT_HASH",
  ownerAuthorityResolverAddress: "VITE_STAGING_OWNER_AUTHORITY_RESOLVER_ADDRESS",
  nftAuthorityResolverAddress: "VITE_STAGING_NFT_AUTHORITY_RESOLVER_ADDRESS",
  facetFallbackFailureHook: "VITE_STAGING_FACET_FALLBACK_FAILURE_HOOK_ADDRESS",
  globalOrganizationRegistry: "VITE_STAGING_GLOBAL_ORGANIZATION_REGISTRY_ADDRESS",
  walletKernelInitializer: "VITE_STAGING_WALLET_KERNEL_INITIALIZER_ADDRESS",
  namespacedCreate3Factory: "VITE_STAGING_NAMESPACED_CREATE3_FACTORY_ADDRESS",
  multicall3Address: "VITE_STAGING_MULTICALL3_ADDRESS",
  barebones4337Facet: "VITE_STAGING_CALIBUR_ENTRY_ADDRESS",
  mockPaymentTokenAddress: "VITE_STAGING_MOCK_PAYMENT_TOKEN_ADDRESS",
  paymentPipelineAddress: "VITE_STAGING_PAYMENT_PIPELINE_ADDRESS",
  payrollEngineAddress: "VITE_STAGING_PAYROLL_ENGINE_ADDRESS",
  payrollManagerAddress: "VITE_STAGING_PAYROLL_MANAGER_ADDRESS",
  payrollTreasuryAddress: "VITE_STAGING_PAYROLL_TREASURY_ADDRESS",
  hoursRuleAddress: "VITE_STAGING_HOURS_RULE_ADDRESS",
  oneTimePaymentAddress: "VITE_STAGING_ONE_TIME_PAYMENT_RULE_ADDRESS",
  commissionRuleAddress: "VITE_STAGING_COMMISSION_RULE_ADDRESS",
  salaryPerSecondRuleAddress: "VITE_STAGING_SALARY_RULE_ADDRESS",
...
function buildStagingBareBonesOverride(): Partial<BareBonesConfiguration> {
  const out: Partial<BareBonesConfiguration> = {};
  for (const key in STAGING_BARE_BONES_CONFIG_ENV_KEYS) {
    const cfgKey = key as Exclude<keyof BareBonesConfiguration, "svrFactoryAddress">;
    const envValue = localEnv(STAGING_BARE_BONES_CONFIG_ENV_KEYS[cfgKey]);
    out[cfgKey] = envValue ?? DEFAULT_BARE_BONES_CONFIG[cfgKey];
  }
  return out;
}
...
> = {
  ...
  [STAGING_CHAIN_ID]: buildStagingBareBonesOverride(),
};
...
const BASE_CHAIN_INFO_MAP: Record<number, ChainInfo> = {
  ...
  [STAGING_CHAIN_ID]: {
    ...
    chainName: "Bare Bones Staging",
    wethAddress: localEnv("VITE_STAGING_WETH_ADDRESS") ?? ethers.constants.AddressZero,
    nativeCurrency: {
      name: "Ethereum",
      symbol: "ETH",
      decimals: 18,
    ...
    logoUrl: anvilLogo,
    rpcUrls: [STAGING_RPC_URL],
    blockExplorerUrls: [STAGING_RPC_URL],
    coinGeckoSlug: "ethereum",
    supportsEip1559: true,
    minPriorityFeeGwei: 1,
    maxFeeMultiplier: {
...
export const NATIVE_ETH_STAGING: TokenInfo = {
  chainId: STAGING_CHAIN_ID,
  address: ethers.constants.AddressZero,
  symbol: "ETH",
  name: "Ether",
...
const BASE_NATIVE_TOKENS_BY_CHAIN: Record<number, TokenInfo> = {
  ...
  [STAGING_CHAIN_ID]: NATIVE_ETH_STAGING,
  [POLYGON_CHAIN_ID]: NATIVE_POLYGON,
  [HYPERLIQUID_CHAIN_ID]: NATIVE_HYPE,
};
